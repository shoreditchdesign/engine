...
function convertRowToArticle(headers, row) {
  try {
    ...
    const title = row[headers.Headline] || "";
    const slug = row[headers.SlugUrl] || "";
    const postId = row[headers.NewsId] || "";
    const category = row[headers.CategoryName] || "";
    const content = row[headers.Content] || "";
    const desc = ""; // No description in your CSV, will be empty
    const published = row[headers.CreatedOnUtc] || "";
    const updatedDate = row[headers.UpdatedOnUtc] || "";
    const tags = ""; // No tags in your CSV
    const isFeatured = row[headers.IsFeatured] || "0";
    const isRecurring = row[headers.IsRecurring] || "0";
    const categoryColor = CATEGORY_COLORS[category] || "#4A90E2";
    const featuredImageBig = "";
    const featuredImageSmall = "";
