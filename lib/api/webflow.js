...
function getWebflowConfig() {
  ...
  if (!siteId) throw new Error('WEBFLOW_SITE_ID environment variable is missing');
  if (!apiToken) throw new Error('WEBFLOW_API_TOKEN environment variable is missing');
...
export async function findItemByPostId(postId) {
  ...
  while (true) {
    ...
    const foundItem = response.items.find(
      item => item.fieldData && item.fieldData[FIELD_MAPPING.postId] === String(postId)
    );
  ...
  return {
    collections: {
      items: {
        ...
        updateItem: (collectionId, itemId, data) => updateItem(collectionId, itemId, data),
        publishItem: (collectionId, itemIds) => publishItems(collectionId, itemIds),
