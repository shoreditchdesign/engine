...
/**
 * Publish entire site to trigger search engine re-crawl
 * @param {string} siteId - Site ID to publish
 * @returns {Promise<object>} - Publish result
 */
export async function publishSite(siteId) {
  return retryWithBackoff(async () => {
    // First get site info to retrieve custom domain IDs
    const siteInfo = await makeWebflowRequest("GET", `/sites/${siteId}`);

    // Extract domain IDs from custom domains
    const domainIds = siteInfo.customDomains?.map((domain) => domain.id) || [];

    if (domainIds.length === 0) {
      throw new Error("No custom domains found for site");
    }

    // Publish to all custom domains
    ...
    logWithTimestamp(
      `Site published successfully to ${domainIds.length} domain(s)`,
      "info",
    );
    return response;
  });
