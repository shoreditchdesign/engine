  ...
  const request = async () => {
    ...
    if (!response.ok) {
      ...
      if ((data && method === "POST") || method === "PATCH") {
        logWithTimestamp(
          `Request data that failed: ${JSON.stringify(data).substring(0, 500)}...`,
          "error",
        );
      }
...
export async function findItemByPostId(postId) {
  ...
  while (true) {
    ...
    if (!response || !response.items || response.items.length === 0) {
      logWithTimestamp(`DEBUG - No items found at offset ${offset}`);
      return null;
    }

    logWithTimestamp(
      `DEBUG - Searching for postId ${postId} (type: ${typeof postId}) in ${response.items.length} items`,
    );
    ...
    const foundItem = response.items.find((item) => {
      if (item.fieldData && item.fieldData[FIELD_MAPPING.postId]) {
        ...
        const match = String(itemPostId) === String(postId);
        if (offset === 0) {
          // Only log first batch to avoid spam
          logWithTimestamp(
            `DEBUG - Comparing ${itemPostId} (type: ${typeof itemPostId}) === ${postId}: ${match}`,
          );
        }
        return match;
      }
