...
class ReferenceManager {
  ...
  async ensureCategoryExists(categoryName, categoryColor = '#000000') {
    ...
    try {
      ...
      if (existingCategories.items && existingCategories.items.length > 0) {
        ...
        console.log(`✓ Category found: "${normalizedName}" (ID: ${categoryId})`);
        return categoryId;
      }
      ...
      console.log(`✓ Category created: "${normalizedName}" (ID: ${newCategory.id})`);
  ...
  async ensureTagExists(tagName) {
    ...
    try {
      ...
      if (existingTags.items && existingTags.items.length > 0) {
        ...
        console.log(`✓ Tag found: "${normalizedName}" (ID: ${tagId})`);
        return tagId;
      }
      ...
      console.log(`✓ Tag created: "${normalizedName}" (ID: ${newTag.id})`);
      return newTag.id;

    } catch (error) {
      console.error(`✗ Error ensuring tag exists: "${tagName}"`, error);
  ...
  async ensureTagsExist(tagNames) {
    ...
    try {
      ...
      console.log(`✓ Processed ${tagIds.length} tags`);
      return tagIds;
