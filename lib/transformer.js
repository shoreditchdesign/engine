...
export function needsUpdate(existingWebflowItem, engineArticle) {
  if (!existingWebflowItem || !existingWebflowItem.fieldData) {
    console.log(`[needsUpdate] No existing item found, needs creation`);
  ...
  if (!webflowUpdatedDate || !engineUpdatedDate) {
    console.log(
      `[needsUpdate] Missing dates - Webflow: ${webflowUpdatedDate}, Engine: ${engineUpdatedDate}`,
    );
  ...
  try {
    ...
    const needsUpdate = engineDate > webflowDate;

    console.log(`[needsUpdate] postId ${engineArticle.postId}:`);
    console.log(`  Webflow date: ${webflowDate.toISOString()}`);
    console.log(`  Engine date:  ${engineDate.toISOString()}`);
    console.log(`  Engine newer? ${needsUpdate}`);
